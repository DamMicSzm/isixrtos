...
#error Platform setup not defined
	...
	extern "C"
	{
		void __attribute__((weak, noinline, externally_visible)) _external_startup(void)
		{
		...
		void __attribute__((weak, noinline, externally_visible)) _external_exit(void)
		{
#ifdef QEMU_NO_RCC_PERIPH
			/* ARM semihosting SYS_EXIT — QEMU quits when run with -semihosting.
			 * Avoid ldr = / __builtin_unreachable: LTO can emit bad branches. */
			__asm volatile(
				"movs r0, %[sys_exit]\n\t"
				"movw r1, %[reason_lo]\n\t"
				"movt r1, %[reason_hi]\n\t"
